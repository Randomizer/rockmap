// RockMap main.js — ASCII-safe version + logo drop-shadow
// Version token for cache-busting in fetches:
var V = "2025-08-01-ascii-2";
...
async function initRockMap() {
  ...
  // 2) Global white marker icon (SVG only)
  ...
  if (hasGeoEvents) {
    ...
    for (var j=0; j<bands.length; j++) {
      ...
      L.marker([lat2, lon2], { title: b.name }).bindPopup(html2, { className: "rockmap-popup", maxWidth: 320 }).addTo(map);
    }
  }

  // 6) Inject small logo bottom-right (with subtle drop-shadow)
  (function injectLogoBR() {
    try {
      var css = "" +
      ...
      img.src = "assets/img/rockmap-logo.png?cv=" + V;
      img.alt = "RockMap";
      img.className = "rockmap-logo";
      a.appendChild(img);
      document.body.appendChild(a);
    } catch (e) {
      console.warn("Logo inject failed", e);
    }
  })();
}
